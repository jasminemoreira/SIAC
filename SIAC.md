...
## DECLARAÇÕES DE LIMITAÇÃO DO EXECUTOR
Quando o Executor é um modelo de linguagem, as seguintes limitações arquiteturais se aplicam. O Operador deve estar ciente delas antes de iniciar qualquer sessão:
1. **Referências bibliográficas**: LLMs não acessam bases indexadas em tempo real e podem fabricar referências com aparência plausível — autores reais, periódicos reais, dados bibliográficos inventados. Toda referência gerada pelo Executor deve ser verificada pelo Operador em base indexada (Scopus, WoS, Google Scholar). Recomenda-se manter repositório local com os PDFs das fontes citadas.
2. **Conteúdo de fontes**: mesmo quando uma referência existe, o Executor pode atribuir a ela afirmações que não correspondem ao conteúdo real — simplificações, distorções ou projeções do argumento do trabalho sobre a fonte. O Operador deve confrontar cada citação com o texto original.
3. **Dados factuais**: números, estatísticas, datas, resoluções institucionais e nomes de programas citados pelo Executor podem ser imprecisos ou fabricados. Dados factuais exigem verificação independente.
4. **Voz autoral**: o Executor produz texto com padrões estilísticos próprios (segunda pessoa, negrito retórico, metalinguagem, anglicismos, dramatização) que podem não corresponder ao registro do Operador. O texto final deve refletir a voz do pesquisador, não a do modelo.
5. **Raciocínio lógico**: o Executor pode justapor dados sem conexão causal e apresentá-los como argumento. O Operador deve verificar se cada dado sustenta a conclusão que o texto dele extrai.
6. **Viés de confirmação do Executor**: quando o Executor gerou o texto original e também conduz a crítica adversarial, pode reproduzir os mesmos vieses nas duas etapas. As dimensões de F2 mitigam parcialmente esse risco ao forçar ângulos específicos, mas o Operador permanece como instância final de julgamento.

Estas limitações não são falhas corrigíveis — são propriedades da arquitetura. Delegar integralmente a execução ao LLM sem verificação equivale a vibe writing com etapas intermediárias.
## COMPORTAMENTO
...
- Integridade de referências 🔴: cada referência citada existe, os dados bibliográficos correspondem à publicação real e a referência está posicionada onde seu conteúdo sustenta a afirmação? Quando o Executor é LLM, esta dimensão é bloqueante por padrão — modelos podem fabricar referências plausíveis. O Operador deve verificar existência em base indexada.
